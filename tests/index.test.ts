...
describe('index', () => {
    describe('prettyoutput general tests', () => {
        it('should output undefined input', () => {
            const output = prettyoutput(undefined)
        ...
        it('should handle maxDeep array value render', () => {
            ...
            assert.strictEqual(
                ...
                [
                    `${colors.green('deep: ')}`,
                    `${colors.green('  - ')}(max depth reached)`,
                    '',
                ].join('\n')
            )
        })
    ...
    describe('utils', () => {
        it('should clear the cache after reaching the cleanup threshold', () => {
            const spaceCount = 5
            for (let i = 0; i < 1000; i++) {
                indent(i)
            }
            const cachedValueBefore = indent(spaceCount)
            indent(1000) // This should trigger the cache cleanup
            const cachedValueAfter = indent(spaceCount)
            assert.strictEqual(cachedValueBefore, cachedValueAfter)
        })
    })
})
